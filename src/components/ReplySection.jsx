...
const ReplySection = ({ currColor, singleMail }) => {
    ...
    return (
        <div className='w-[800px] h-[600px] border border-gray-700 flex flex-col'>
            <div className='relative w-full flex flex-col'>
                <div className='flex h-[70px] border border-gray-700 py-3 pl-4'>
                    ...
                    <div className='flex justify-end gap-4 h-8 my-2 w-full'>
                        <div className={`flex items-center gap-2 p-2 border border-gray-700 rounded ${currColor ? 'bg-[#222426]' : 'bg-[#fff]'}`}>
                            <p className='w-3 h-3 rounded-3xl bg-yellow-600'></p>
                            <p className='text-[12px]'>Meeting Completed</p>
                            <ChevronDown />
                        </div>
                        <div className={`flex items-center p-2 border border-gray-700 rounded ${currColor ? 'bg-[#222426]' : 'bg-[#fff]'}`}>
                ...
                {showReply && (
                    <div className={`absolute mt-[134px] w-[768px] ml-4 rounded-2xl z-10 overflow-hidden h-[450px] ${currColor ? 'bg-[#141517]' : 'bg-white'} border border-gray-700`}>
                        <SendReply currColor={currColor} singleMail={singleMail[0]} handleCancel={() => setShowReply(false)} />
                    </div>
                )}
            </div>
            <div className='w-[100px] h-10 bg-[#4B63DD] flex items-center ml-4 mb-3 rounded gap-1 justify-center'>
                <Reply color='white' />
